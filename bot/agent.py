...
class TextilProAgent:
    ...
    def _fallback_response(self, user_message: str) -> str:
        """Fallback ответы когда LLM недоступны"""
        user_message_lower = user_message.lower()
        
        if any(word in user_message_lower for word in ['привет', 'hello', 'hi', 'здравствуй']):
            return "👋 Привет! Меня зовут Анастасия, я консультант Textile Pro. Чем могу помочь?"
        elif any(word in user_message_lower for word in ['цена', 'стоимость', 'сколько']):
            return "💰 Цены зависят от объема и типа продукции. Расскажите подробнее о ваших потребностях - количество, тип одежды, материалы."
        elif any(word in user_message_lower for word in ['ткань', 'материал', 'хлопок', 'полиэстер']):
            return "🧵 У нас широкий выбор тканей и материалов! Расскажите какой именно материал вас интересует - хлопок, полиэстер, смесовые ткани?"
        elif any(word in user_message_lower for word in ['китай', 'china', 'производство']):
            return "🏭 Мы работаем с проверенными фабриками в Китае, Индии и Бангладеш. Обеспечиваем полный цикл производства с контролем качества."
        elif any(word in user_message_lower for word in ['доставка', 'логистика', 'shipping']):
            return "🚢 Организуем доставку морским, авиа и железнодорожным транспортом. Время доставки 15-45 дней в зависимости от способа."
        elif any(word in user_message_lower for word in ['качество', 'контроль', 'проверка']):
            return "✅ У нас строгий контроль качества на всех этапах. Предоставляем фото отчеты, можем организовать инспекцию третьей стороной."
        else:
            return f"Поняла ваш вопрос! Отличный вопрос о текстильном производстве.\n\nПодготовлю детальный ответ специально для вас. Минуточку!\n\nАнастасия, Textil PRO"
